...
<h2>
	<spring:message code="gaac.search.manage" />
</h2>
...
<c:if test="${fn:length(gaacMemberHistoryList) > 0}">
	<b class="boxHeader" ><openmrs:message code="gaac.search.history" /></b>
	<div class="box">
		<table class="openmrsSearchTable" style="width: 100%;" cellpadding="2"
			cellspacing="0">
			<thead>
				<tr style="" dojoattachpoint="headerRow">
					<th><openmrs:message code="gaac.gaac" javaScriptEscape="true" /></th>
			...
			<c:forEach var="member" items="${gaacMemberHistoryList}"
				...
				<tr
					...
					<td><c:if test="${member.gaac.voided==false}"><a href="addNewGaac.form?gaacId=${member.gaac.gaacId}"><c:out value="${member.gaac.name}" /></a></c:if>
					<c:if test="${member.gaac.voided==true}"><c:out value="${member.gaac.name}" /></c:if>
					</td>
					<td><c:out value="${member.gaac.location.name}" /></td>
