...
public class GaacValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (gaac == null) {
			...
			ValidationUtils.rejectIfEmptyOrWhitespace(errors, "gaacIdentifier", "gaac.error.identifier");
			...
			if (gaac.getGaacId() == null) {
				...
				FamilyMember familyMemberValidator = gs.getFamilyMemberByMember(gaac.getFocalPatient());

				GaacMember gaacMemberValidator = gs.getGaacMemberByMember(gaac.getFocalPatient());
				...
				if (temp != null) {
					errors.rejectValue("gaacIdentifier", "gaac.error.identifier.inuse");
				}
			} else {
				...
				if (gaac.getCrumbled() == true) {
					...
					if (gaac.getDateCrumbled() == null) {
						ValidationUtils.rejectIfEmptyOrWhitespace(errors, "dateCrumbled", "gaac.error.dateCrumbled");
					} else if (gaac.getDateCrumbled().after(new Date())) {
						errors.rejectValue("dateCrumbled", "gaac.manage.error.DateCrumbled");
					}
			...
			if (gaac.getLocation() == null) {
				ValidationUtils.rejectIfEmptyOrWhitespace(errors, "location", "gaac.error.location");
			}
			if (gaac.getAffinity() == null) {
				ValidationUtils.rejectIfEmptyOrWhitespace(errors, "affinity", "gaac.error.affinity");
			}
